...
async function create(user, outlet) {
    return new Promise(async (resolve, reject) => {
        visit = await visitModel.create({ //query today up to tonight
            date: new Date(),
            user: user,
...
async function get(user, outlet) {
    return new Promise(async (resolve, reject) => {
        ...
        visit = await visitModel.findOne({ //query today up to tonight
            date: {
                $gte: start, 
                $lt: end
            },
...
async function hasVisit(user, outlet) {
    return new Promise(async (resolve, reject) => {
        visit = await visitModel.find({ //query today up to tonight
            project: user.project,
            user: user,
            outlet: outlet
        })
...
async function allVisits(user, outlet) {
    return new Promise(async (resolve, reject) => {
        visits = await visitModel.find({ //query today up to tonight
            project: user.project,
            user: user,
            outlet: outlet
        })
